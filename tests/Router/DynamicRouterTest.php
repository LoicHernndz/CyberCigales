 ...
 *
 * @param string $segment Le segment d'URL en kebab-case
 * @return string Le segment converti en PascalCase
 */
 ...
 *
 * @param string $uri L'URI à résoudre
 * @return array{0: string|null, 1: array<int, string>} [className, params] ou [null, []] si introuvable
 */
...
echo "\n=== Tests de conversion kebab-case → PascalCase (edge cases) ===\n\n";

test("segment vide '' → ''", kebabToPascal('') === '');
test("'a' → 'A'", kebabToPascal('a') === 'A');
test("'ABC' → 'ABC' (déjà majuscule)", kebabToPascal('ABC') === 'ABC');
test("'a-b-c' → 'ABC' (segments d'une lettre)", kebabToPascal('a-b-c') === 'ABC');
test("'mon-segment-long' → 'MonSegmentLong'", kebabToPascal('mon-segment-long') === 'MonSegmentLong');
...
test(
    "/user/new-password → Controllers\\User\\NewPassword",
    resolveUri('/user/new-password')[0] === 'Controllers\\User\\NewPassword'
);

test(
    "/user/edit → Controllers\\User\\Edit",
    resolveUri('/user/edit')[0] === 'Controllers\\User\\Edit'
);

test(
    "/user/delete → Controllers\\User\\Delete",
...
test(
    "/game/cypher-rush → Controllers\\Game\\CypherRush",
    resolveUri('/game/cypher-rush')[0] === 'Controllers\\Game\\CypherRush'
);

test(
    "/game/frequency → Controllers\\Game\\Frequency",
    resolveUri('/game/frequency')[0] === 'Controllers\\Game\\Frequency'
);

test(
    "/game/phishing → Controllers\\Game\\Phishing",
    resolveUri('/game/phishing')[0] === 'Controllers\\Game\\Phishing'
);

test(
    "/data-breach/check → Controllers\\DataBreach\\Check",
...
test(
    "/instagram → Controllers\\Instagram\\Index",
    resolveUri('/instagram')[0] === 'Controllers\\Instagram\\Index'
);

test(
    "/bash → Controllers\\Bash\\Index",
    resolveUri('/bash')[0] === 'Controllers\\Bash\\Index'
);

test(
    "/minigames → Controllers\\Minigames\\Index",
...
test(
    "/bash/exec → Controllers\\Bash\\Exec",
    resolveUri('/bash/exec')[0] === 'Controllers\\Bash\\Exec'
);

test(
    "/instagram/chat/response → Controllers\\Instagram\\Chat\\Response",
...
test(
    "/instagram/user/mel_133 → Controllers\\Instagram\\User avec params",
    $result[0] === 'Controllers\\Instagram\\User' && $result[1] === ['mel_133']
);

$result = resolveUri('/instagram/user/mel_133/chat');
test(
    "/instagram/user/mel_133/chat → Controllers\\Instagram\\User avec params [mel_133, chat]",
...
test(
    "/route-inexistante → null (404)",
    resolveUri('/route-inexistante')[0] === null
);

test(
    "/foo/bar/baz → null (404)",
    resolveUri('/foo/bar/baz')[0] === null
);

echo "\n=== Tests edge cases (robustesse du routeur) ===\n\n";

test(
    "Trailing slash /plan/ → Controllers\\Plan (même résultat que /plan)",
    resolveUri('/plan/')[0] === 'Controllers\\Plan'
);

test(
    "Double slash //plan → Controllers\\Plan",
    resolveUri('//plan')[0] === 'Controllers\\Plan'
);

test(
    "/lecon/cesar/ (trailing slash) → Controllers\\Lecon\\Cesar",
    resolveUri('/lecon/cesar/')[0] === 'Controllers\\Lecon\\Cesar'
);

$result = resolveUri('/instagram/user/mel_133/');
test(
    "/instagram/user/mel_133/ (trailing slash) → même résultat avec params",
    $result[0] === 'Controllers\\Instagram\\User' && $result[1] === ['mel_133']
);

test(
    "Un seul segment inconnu → null (404)",
    resolveUri('/zzz-inconnu')[0] === null
);

test(
    "Sous-chemin profond inconnu → null (404)",
    resolveUri('/a/b/c/d/e/f')[0] === null
);
